...
where $E$ is a chosen embedding function and $\mathbb{L}$ is a subset of the overall function space of possible $\Lambda$ values. The choice of embedding necessarily restricts and dictates the systems which the system can deal with thus its choice is crucial in the design of any system.

For example one could choose to model possibly damped harmonic oscillators using the embeddings function,

\begin{equation}
\begin{gathered}
  E_{\text{DHO}} : \R^3 \to \mathbb{L} \\
  E(m, k, \lambda) = \frac12 m\dot\vbq^2 - \frac12 m\dot\vbq - \lambda\dot \vbq_+\vbq_-
\end{gathered}
\end{equation}

where we would place some additional requirements on our embedding domain based on our physical knowledge of the system, for example that the mass $m$ and spring constant $k$.
